...
struct CholeskyDecomposition : public OpRewritePattern<mhlo::CholeskyOp> {
  using OpRewritePattern<mhlo::CholeskyOp>::OpRewritePattern;

  LogicalResult matchAndRewrite(mhlo::CholeskyOp op,
                                PatternRewriter &rewriter) const override {
    llvm::dbgs() << "XXX: matchAndReWrite\n";
    auto resultTy = rewriter.getF64FloatAttr(1).getType();
    //auto resultTy = op.getType().cast<RankedTensorType>();
    auto lhs = op.getA();
    auto rhs = rewriter.create<arith::ConstantOp>(
      op.getLoc(),
      rewriter.getF64FloatAttr(1));
    //rewriter.replaceOpWithNewOp<mhlo::MulOp>(op, resultTy, lhs, rhs);
    //rewriter.replaceOpWithNewOp<arith.ConstantOp>(
    //  op, resultTy, rewriter.getF64FloatAttr(1));

    // Create zero matrix
    Value zero = rewriter.create<arith::ConstantOp>(
      op.getLoc(), rewriter.getZeroAttr(op.getType().cast<RankedTensorType>().getElementType()));
    ...
    // Do a while operation. While variable is less than 3 is cond
    ...
    //SmallVector<Value, 2> init_values { 1 };
    int value_count = 1;
...

    {
      ...
      auto upper_limit = builder->create<mhlo::ConstantOp>(
          loc, builder->getI32IntegerAttr(3));
    ...
    //auto mul_op = builder->create<mhlo::MulOp>(loc, emptyTensor, while_op);


    //
    // End new while attempt
    //
    ///////////////////
    ...
    //Value zeroConst = rewriter.create<arith::ConstantIndexOp>(0);
    //SmallVector<Value> whileInitArgs{zeroConst};
    //SmallVector<Type> whileArgTypes;
    ////for (auto val : whileInitArgs) whileArgTypes.push_back(val.getType());

    //SmallVector<Value> operands;
    //operands.push_back([&](OpBuilder& rewriter, Location loc, ValueRange args) {
    //    Value current = args[0];
    //    Value three = rewriter.create<arith::ConstantIndexOp>(loc, 3);
    //    rewriter.create<mhlo::CompareOp>(
    //        loc, current, three, mhlo::ComparisonDirection::NE);
    //});
    //operands.push_back([&](OpBuilder& afterBuilder, Location afterLoc, ValueRange args) {
    //});

    //auto whileOp = rewriter.create<mhlo::WhileOp>(
    //  op.getLoc(), whileArgTypes, operands);
    ...
    // Set a value in the zero matrix and increment the variable

    //Value zeroTensor =
    //    rewriter.create<mhlo::FillOp>(op.getLoc(), zero, emptyTensor).getResult(0);

    // Replace with constant
    //auto result = rewriter.create<arith::ConstantIntOp>(op.getLoc(), 32, 32);
    //rewriter.replaceOp(op, {result});

    rewriter.replaceOp(op, while_op);
    //rewriter.replaceOp(op, {while_op});
...
struct MHLOToMHLOPreprocessingPass
  ...
  void getDependentDialects(DialectRegistry &registry) const override {
    registry.insert<shape::ShapeDialect, mhlo::MhloDialect,
                    tensor::TensorDialect, chlo::ChloDialect>();
  }
  ...
  void runOnOperation() override {
    ...
    // Decompose Cholesky
    llvm::dbgs() << "XXX: decompose\n";
    patterns.insert<CholeskyDecomposition>(context);
